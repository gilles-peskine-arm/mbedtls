...
LOCAL_CFLAGS = $(WARNING_CFLAGS) -I$(MBEDTLS_TEST_PATH)/include \
               -I$(MBEDTLS_FRAMEWORK)/tests/include \
...
MBEDTLS_CORE_TEST_OBJS := $(patsubst %.c,%.o,$(wildcard \
    $(MBEDTLS_FRAMEWORK)/tests/src/*.c \
    $(MBEDTLS_FRAMEWORK)/tests/src/drivers/*.c \
  ))
...
MBEDTLS_CORE_TEST_OBJS := $(filter-out \
    $(MBEDTLS_FRAMEWORK)/tests/src/psa_crypto_stubs.o, $(MBEDTLS_CORE_TEST_OBJS)\
  )
endif
