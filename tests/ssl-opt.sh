...
# Tests for CA list suppression

run_test    "Authentication: CA suppression disabled (default)" \
            "$P_SRV debug_level=3 auth_mode=required" \
...
run_test    "Authentication: CA suppression enabled" \
...
run_test    "Authentication: CA suppression enabled, client self signed" \
            "$P_SRV debug_level=3 auth_mode=required cert_req_ca_list=0" \
            "$P_CLI debug_level=3 crt_file=data_files/server5-selfsigned.crt \
             key_file=data_files/server5.key" \
            1 \
