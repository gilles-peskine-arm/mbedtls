...
if [ $FORCE -eq 1 ]; then
    ...
    clobbered_files="include/mbedtls/config.h"
    if ! grep -q CMAKE Makefile; then
        # Warn about edited makefiles, but not if they were generated by CMake
        clobbered_files="$clobbered_files Makefile library/Makefile programs/Makefile tests/Makefile"
    fi
    if ! git diff-files --name-only --exit-code $clobbered_files; then
        err_msg "Warning - the files(s) above have been edited. "
