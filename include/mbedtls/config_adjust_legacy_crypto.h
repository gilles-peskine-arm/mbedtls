 ...
 * MBEDTLS_A but not MBEDTLS_B when A requires B results in a compilation
 * error. However, we do automatically enable certain options in some
 * circumstances. One case is if MBEDTLS_B is an internal option used
 * to identify parts of a module that are used by other module, and we
 * don't want to make the symbol MBEDTLS_B part of the public API.
 ...
 * configurations that explicitly activate MBEDTLS_A but not MBEDTLS_B.
 */
/*
