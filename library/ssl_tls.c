...
static int ssl_handshake_init( mbedtls_ssl_context *ssl )
{
    ...
    {
        uint32_t in_buf_new_len = MBEDTLS_SSL_IN_BUFFER_LEN;
        uint32_t out_buf_new_len = MBEDTLS_SSL_OUT_BUFFER_LEN;
        int modified = 0;
        ...
        if( ssl->in_buf != NULL )
        {
            ...
            if( ssl->in_buf_len < in_buf_new_len )
            {
        ...
        if( ssl->out_buf != NULL )
        {
            ...
            if( ssl->out_buf_len < out_buf_new_len )
            {
...
void mbedtls_ssl_handshake_free( mbedtls_ssl_context *ssl )
{
     ...
     * empty at this stage */
    {
        uint32_t in_buf_new_len = mbedtls_ssl_get_input_buflen( ssl );
        uint32_t out_buf_new_len = mbedtls_ssl_get_output_buflen( ssl );
        int modified = 0;
        size_t written_in = 0, iv_offset_in = 0, len_offset_in = 0;
        size_t written_out = 0, iv_offset_out = 0, len_offset_out = 0;
        if( ssl->in_buf != NULL )
        {
            ...
            iv_offset_in = ssl->in_iv - ssl->in_buf;
            len_offset_in = ssl->in_len - ssl->in_buf;
            if( ssl->in_buf_len > in_buf_new_len && ssl->in_left < in_buf_new_len )
            {
                if( resize_buffer( &ssl->in_buf, in_buf_new_len, &ssl->in_buf_len ) != 0 )
                {
                    MBEDTLS_SSL_DEBUG_MSG( 1, ( "input buffer resizing failed - out of memory" ) );
        ...
        if( ssl->out_buf != NULL )
        {
            ...
            iv_offset_out = ssl->out_iv - ssl->out_buf;
            len_offset_out = ssl->out_len - ssl->out_buf;
            if( ssl->out_buf_len > out_buf_new_len && ssl->out_left < out_buf_new_len )
            {
                if( resize_buffer( &ssl->out_buf, out_buf_new_len, &ssl->out_buf_len ) != 0 )
                {
                    MBEDTLS_SSL_DEBUG_MSG( 1, ( "output buffer resizing failed - out of memory" ) );
