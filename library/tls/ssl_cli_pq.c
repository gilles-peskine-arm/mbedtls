...
#if defined(KEM_REWORK)
int ssl_pkwrap_write_client_key_exchange( mbedtls_ssl_context *ssl ); 
int ssl_pkwrap_parse_server_key_exchange( mbedtls_ssl_context *ssl ); 
#endif 
 ...
 */
    ...
    defined(MBEDTLS_KEM_WITH_CERT_ENABLED)
                                               ...
                                               size_t *olen )
{
    ...
    *olen = 0;
    ...
    {
    #ifdef MBEDTLS_PQ_C
        mbed_md_alg_e mdtype = mbedtls_ssl_hash_from_md_alg( *md );
        if (mdtype >= MBEDTLS_MD_SHA384)
        {
            sig_alg_list[sig_alg_len++] = mdtype;
            sig_alg_list[sig_alg_len++] = MBEDTLS_SSL_SIG_HYBRID;
            sig_alg_list[sig_alg_len++] = mdtype;
            sig_alg_list[sig_alg_len++] = MBEDTLS_SSL_SIG_PQ;
        }
    #endif // #ifdef MBEDTLS_PQ_C
    #if defined(MBEDTLS_ECDSA_C)
    ...
    *olen = 6 + sig_alg_len;
    ...
    return( 0 );
          ...
          MBEDTLS_KEM_WITH_CERT_ENABLED */

#if defined(MBEDTLS_ECDH_C) || defined(MBEDTLS_ECDSA_C) || \
    defined(MBEDTLS_KEM_ECJPAKE_ENABLED)
...
static int ssl_write_supported_point_formats_ext( mbedtls_ssl_context *ssl, uint8_t *buf, const uint8_t *end, size_t *olen )
{
    uint8_t *p = buf;
...
{
    int ret = MBEDTLS_ERR_ERROR_CORRUPTION_DETECTED;
    uint8_t *p = ssl->handshake->randbytes;
#if defined(MBEDTLS_HAVE_TIME)
    mbedtls_time_t t;
#endif
...
static int ssl_write_client_hello( mbedtls_ssl_context *ssl )
{
  ...
  #if defined(MBEDTLS_ECDH_C) || defined(MBEDTLS_ECDSA_C) || defined(MBEDTLS_KEM_ECJPAKE_ENABLED)
    int uses_ec = 0;
  #endif
...
#if 0 
    {
        // TODO: Make this process pktype agnostic (using a list of registered OPK types) 
        // Loop through all selected ciphersuites again, select which PK algorithms are used (in selected_pktype_list) 
        // Then for all pktype that are used, call mbed_pk_ssl_ext() to add the ssl extension data 
        mbed_hybpk_t  selected_pktype_list[MBEDTLS_PK_LAST] = {0}; 
        mbed_hybpk_t  pktype; 
        uint32_t       comb_pktype; 
        memset(selected_pktype_list, 0, sizeof(selected_pktype_list)); 
        for( i = 0; ciphersuites[i] != 0; i++ )
        {
            ciphersuite_info = mbedtls_ssl_ciphersuite_from_id( ciphersuites[i] );
            if( ssl_validate_ciphersuite( ciphersuite_info, ssl, ssl->conf->min_minor_ver, ssl->conf->max_minor_ver ) != 0 )
                continue;
            comb_pktype = mbedtls_ssl_get_ciphersuite_kem_pk_alg(ciphersuite_info); 
            while (comb_pktype > 0 )
            {
                pktype = comb_pktype && 0xff; // TODO hardcoded 0xff 
                if ( pktype < MBEDTLS_PK_LAST)
                    selected_pktype_list[pktype] = 1; 
                comb_pktype = (comb_pktype >> 8); 
            }
        }
        for( pktype = MBEDTLS_PK_NONE; i< MBEDTLS_PK_LAST; pktype++ )
        {
            if (selected_pktype_list[pktype] == 0 )
                continue 
            printf("<<PQ>>: write extension for pktupe %d\n", (int) pktype); 
            mbed_pk_write_ssl_ext( pktype, p + 2 + ext_len, end, &olen); 
            if (olen <= 0)
                continue;

            MBEDTLS_SSL_DEBUG_MSG( 3, ("client hello, added pktype = %d ssl extension") );
            ext_len += olen; 
        }        
        ssl->conf->curve_list
    }
  ...
  #endif // #if defined(MBEDTLS_ECDH_C) || defined(MBEDTLS_ECDSA_C) || defined(MBEDTLS_KEM_ECJPAKE_ENABLED)
#endif // #ifdef MBEDTLS_PQ_C
...
{
    const uint8_t *p = ssl->in_msg + mbedtls_ssl_hs_hdr_len( ssl );
...
static int ssl_parse_server_hello( mbedtls_ssl_context *ssl )
{
    ...
    if( suite_info->key_exchange == MBEDTLS_KEM_ECDHE_ECDSA && ssl->minor_ver == MBEDTLS_SSL_MINOR_VERSION_3 )
    {
    ...
    while( ext_len )
    {
        ...
        case MBEDTLS_TLS_EXT_SUPPORTED_POINT_FORMATS:
            ...
            break;
      #endif /* MBEDTLS_ECDH_C || MBEDTLS_ECDSA_C || MBEDTLS_KEM_ECJPAKE_ENABLED */
...
#if defined(MBEDTLS_KEM_DHE_RSA_ENABLED) || defined(MBEDTLS_KEM_DHE_PSK_ENABLED)

static int ssl_parse_server_dh_params( mbedtls_ssl_context *ssl, uint8_t **p, uint8_t *end )
{
...
#endif /* #if defined(MBEDTLS_KEM_DHE_RSA_ENABLED) || defined(MBEDTLS_KEM_DHE_PSK_ENABLED) */

#if 0
//#if !defined(KEM_REWORK)

#if defined(MBEDTLS_KEM_ECDHE_RSA_ENABLED) ||                     \
    defined(MBEDTLS_KEM_ECDHE_ECDSA_ENABLED) ||                   \
    defined(MBEDTLS_KEM_ECDHE_PSK_ENABLED) ||                     \
    defined(MBEDTLS_KEM_ECDH_RSA_ENABLED) ||                      \
    defined(MBEDTLS_KEM_ECDH_ECDSA_ENABLED)
...
#endif /* MBEDTLS_KEM_ECDHE_RSA_ENABLED ||
          MBEDTLS_KEM_ECDHE_ECDSA_ENABLED ||
          MBEDTLS_KEM_ECDHE_PSK_ENABLED ||
          MBEDTLS_KEM_ECDH_RSA_ENABLED ||
          MBEDTLS_KEM_ECDH_ECDSA_ENABLED */

#if defined(MBEDTLS_USE_PSA_CRYPTO) &&                           \
        ( defined(MBEDTLS_KEM_ECDHE_RSA_ENABLED) ||     \
          defined(MBEDTLS_KEM_ECDHE_ECDSA_ENABLED) )
                                         ...
                                         uint8_t *end )
{
     ...
     * } ServerECDHParams;
     */
    if( ( ret = mbedtls_ecdh_read_params( &ssl->handshake->ecdh_ctx, (const uint8_t **) p, end ) ) != 0 )
    {
...
#endif /* MBEDTLS_KEM_SOME_PSK_ENABLED */
#endif // #if !defined(KEM_REWORK)

#if defined(MBEDTLS_KEM_RSA_ENABLED) ||                           \
    defined(MBEDTLS_KEM_RSA_PSK_ENABLED)
/*
...
static int ssl_write_encrypted_pms( mbedtls_ssl_context *ssl, size_t offset, size_t *olen, size_t pms_offset )
{
    ...
    {
        printf("%s(): unsupported RSA only use of mbed_pk_can_do() !!!!!\n"); fflush(stdout); 
        MBEDTLS_SSL_DEBUG_MSG( 1, ( "certificate key type mismatch" ) );
        return( MBEDTLS_ERR_SSL_PK_TYPE_MISMATCH );
    }

    if( ( ret = mbed_pk_encap( peer_pk,
    ...
    return( 0 );
          ...
          MBEDTLS_KEM_RSA_PSK_ENABLED */

#if defined(MBEDTLS_SSL_PROTO_TLS1_2)
#if defined(MBEDTLS_KEM_DHE_RSA_ENABLED) ||                       \
    defined(MBEDTLS_KEM_ECDHE_RSA_ENABLED) ||                     \
    defined(MBEDTLS_KEM_ECDHE_ECDSA_ENABLED)
                                          ...
                                          mbed_hybpk_t *pk_alg )
{
    ...
    *pk_alg = MBEDTLS_PK_NONE;
     ...
     */
    MBEDTLS_SSL_DEBUG_MSG( 3,( "server uses SignatureAlgorithm: %d", (*p)[1] ) );
...
#endif /* MBEDTLS_KEM_DHE_RSA_ENABLED ||
          MBEDTLS_KEM_ECDHE_RSA_ENABLED ||
          MBEDTLS_KEM_ECDHE_ECDSA_ENABLED */
#endif /* MBEDTLS_SSL_PROTO_TLS1_2 */

#if defined(MBEDTLS_KEM_ECDH_RSA_ENABLED) || \
    defined(MBEDTLS_KEM_ECDH_ECDSA_ENABLED)
static int ssl_get_ecdh_params_from_cert( mbedtls_ssl_context *ssl )
{
    ...
    if( ! mbed_pk_can_do_single( peer_pk, MBEDTLS_PK_ECKEY ) )
    {
        MBEDTLS_SSL_DEBUG_MSG( 1, ( "server key not ECDH capable" ) );
        return( MBEDTLS_ERR_SSL_PK_TYPE_MISMATCH );
    }

    peer_key = mbed_get_eckey_ctx( *peer_pk );
...
#endif /* MBEDTLS_KEM_ECDH_RSA_ENABLED) ||
          MBEDTLS_KEM_ECDH_ECDSA_ENABLED */
...
static int ssl_parse_server_key_exchange( mbedtls_ssl_context *ssl )
{
...
#if defined(MBEDTLS_KEM_RSA_ENABLED)
    if( ciphersuite_info->key_exchange == MBEDTLS_KEM_RSA )
    {
...
#if defined(MBEDTLS_KEM_ECDH_RSA_ENABLED) || \
    defined(MBEDTLS_KEM_ECDH_ECDSA_ENABLED)
    if( ciphersuite_info->key_exchange == MBEDTLS_KEM_ECDH_RSA ||
        ciphersuite_info->key_exchange == MBEDTLS_KEM_ECDH_ECDSA )
    {
    ...
    {
        if( ciphersuite_info->key_exchange == MBEDTLS_KEM_PSK ||
            ciphersuite_info->key_exchange == MBEDTLS_KEM_RSA_PSK )
        {
...
start_processing:
    ...
    if( ciphersuite_info->key_exchange == MBEDTLS_KEM_PSK ||
        ciphersuite_info->key_exchange == MBEDTLS_KEM_RSA_PSK ||
        ciphersuite_info->key_exchange == MBEDTLS_KEM_DHE_PSK ||
        ciphersuite_info->key_exchange == MBEDTLS_KEM_ECDHE_PSK )
    {
    ...
    if( ciphersuite_info->key_exchange == MBEDTLS_KEM_PSK ||
        ...
        ; /* nothing more to do */
          ...
          MBEDTLS_KEM_RSA_PSK_ENABLED */
#if defined(MBEDTLS_KEM_DHE_RSA_ENABLED) ||                       \
    defined(MBEDTLS_KEM_DHE_PSK_ENABLED)
    if( ciphersuite_info->key_exchange == MBEDTLS_KEM_DHE_RSA ||
        ciphersuite_info->key_exchange == MBEDTLS_KEM_DHE_PSK )
    {
...
#endif /* MBEDTLS_KEM_DHE_RSA_ENABLED ||
          MBEDTLS_KEM_DHE_PSK_ENABLED */
#if defined(MBEDTLS_USE_PSA_CRYPTO) &&                           \
        ( defined(MBEDTLS_KEM_ECDHE_RSA_ENABLED) ||     \
          defined(MBEDTLS_KEM_ECDHE_ECDSA_ENABLED) )
    if( ciphersuite_info->key_exchange == MBEDTLS_KEM_ECDHE_RSA ||
        ciphersuite_info->key_exchange == MBEDTLS_KEM_ECDHE_ECDSA )
    {
    ...
    if( ciphersuite_info->key_exchange == MBEDTLS_KEM_ECDHE_RSA ||
        ciphersuite_info->key_exchange == MBEDTLS_KEM_ECDHE_PSK ||
      #ifdef MBEDTLS_PQ_C 
        ciphersuite_info->key_exchange == MBEDTLS_KEM_PQKEM_PQDSA || 
        ciphersuite_info->key_exchange == MBEDTLS_KEM_HYBKEM_HYBDSA || 
      #endif // MBEDTLS_PQ_C
        ciphersuite_info->key_exchange == MBEDTLS_KEM_ECDHE_ECDSA )
    {
        if( ssl_parse_server_ecdh_params( ssl, &p, end ) != 0 )
        {
          ...
          MBEDTLS_KEM_ECDHE_ECDSA_ENABLED */
#if defined(MBEDTLS_KEM_ECJPAKE_ENABLED)
    if( ciphersuite_info->key_exchange == MBEDTLS_KEM_ECJPAKE )
    {
    ...
    if( mbedtls_ssl_ciphersuite_uses_server_signature( ciphersuite_info ) )
    {
        ...
        mbed_hybpk_t pk_alg = MBEDTLS_PK_NONE;
         ...
         * Verify signature
         */
        if( !mbed_pk_can_do( peer_pk, pk_alg ) )
        {
            MBEDTLS_SSL_DEBUG_MSG( 1, ( "bad server key exchange message" ) );
            mbedtls_ssl_send_alert_message(
                ssl,
        ...
        if( ( ret = mbed_pk_verify_restartable( peer_pk,
                        md_alg, hash, hashlen, p, sig_len, rs_ctx ) ) != 0 )
        {
            ...
            MBEDTLS_SSL_DEBUG_RET( 1, "mbed_pk_verify", ret );
#if defined(MBEDTLS_SSL_ECP_RESTARTABLE_ENABLED)
            if( ret == MBEDTLS_ERR_ECP_IN_PROGRESS )
                ret = MBEDTLS_ERR_SSL_CRYPTO_IN_PROGRESS;
#endif
...
#endif // #ifdef KEM_REWORK 


#if ! defined(MBEDTLS_KEM_CERT_REQ_ALLOWED_ENABLED)
...
#else /* MBEDTLS_KEM_CERT_REQ_ALLOWED_ENABLED */
static int ssl_parse_certificate_request( mbedtls_ssl_context *ssl )
{
    int ret = MBEDTLS_ERR_ERROR_CORRUPTION_DETECTED;
    uint8_t *buf;
...
#endif /* MBEDTLS_KEM_CERT_REQ_ALLOWED_ENABLED */
...
static int ssl_write_client_key_exchange( mbedtls_ssl_context *ssl )
{
        ...
        ciphersuite_info->key_exchange == MBEDTLS_KEM_ECDHE_ECDSA )
    {
        ...
        uint8_t own_pubkey[MBEDTLS_PSA_MAX_EC_PUBKEY_LENGTH];
        size_t own_pubkey_len;
        uint8_t *own_pubkey_ecpoint;
    ...
    if( ciphersuite_info->key_exchange == MBEDTLS_KEM_ECDHE_RSA ||
        ciphersuite_info->key_exchange == MBEDTLS_KEM_ECDHE_ECDSA ||
        ciphersuite_info->key_exchange == MBEDTLS_KEM_ECDH_RSA ||
      #ifdef MBEDTLS_PQ_C 
        ciphersuite_info->key_exchange == MBEDTLS_KEM_PQKEM_PQDSA || 
        ciphersuite_info->key_exchange == MBEDTLS_KEM_HYBKEM_HYBDSA || 
      #endif // MBEDTLS_PQ_C
...
ecdh_calc_secret:
    ...
    if( mbedtls_ssl_ciphersuite_uses_psk( ciphersuite_info ) )
    {
        ...
        if( ciphersuite_info->key_exchange == MBEDTLS_KEM_RSA_PSK )
        {
#if defined(MBEDTLS_USE_PSA_CRYPTO)
        ...
        if( ciphersuite_info->key_exchange == MBEDTLS_KEM_DHE_PSK )
        {
#if defined(MBEDTLS_USE_PSA_CRYPTO)
            /* Opaque PSKs are currently only supported for PSK-only suites. */
            if( ssl_conf_has_static_raw_psk( ssl->conf ) == 0 )
        ...
        if( ciphersuite_info->key_exchange == MBEDTLS_KEM_ECDHE_PSK )
        {
#if defined(MBEDTLS_USE_PSA_CRYPTO)
            /* Opaque PSKs are currently only supported for PSK-only suites. */
            if( ssl_conf_has_static_raw_psk( ssl->conf ) == 0 )
...
#if defined(MBEDTLS_USE_PSA_CRYPTO) &&          \
    defined(MBEDTLS_KEM_PSK_ENABLED)
        if( ciphersuite_info->key_exchange == MBEDTLS_KEM_PSK &&
...
sign:
    ...
    if( ( ret = mbed_pk_sign_restartable( mbedtls_ssl_own_key( ssl ), md_alg, hash_start, hashlen, 
                                          ssl->out_msg + 6 + offset, out_buf_len - 6 - offset, &n, rs_ctx) ) != 0 )
    {
        MBEDTLS_SSL_DEBUG_RET( 1, "mbed_pk_sign", ret );
#if defined(MBEDTLS_SSL_ECP_RESTARTABLE_ENABLED)
        if( ret == MBEDTLS_ERR_ECP_IN_PROGRESS )
            ret = MBEDTLS_ERR_SSL_CRYPTO_IN_PROGRESS;
#endif
...
int mbedtls_ssl_handshake_client_step( mbedtls_ssl_context *ssl )
{
    ...
    case MBEDTLS_SSL_SERVER_KEY_EXCHANGE:
        ret = ssl_pkwrap_parse_server_key_exchange(ssl);
        break;
    ...
    case MBEDTLS_SSL_CLIENT_KEY_EXCHANGE:
        ret = ssl_pkwrap_write_client_key_exchange(ssl);
        break;
