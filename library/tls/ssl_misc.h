...
typedef struct ssl_kemctx_s
{
    mbed_pktop_t     *kem_key; 
    uint32_t          hybpk_alg; 

    mbed_pktop_t     *kem_peerkey; 

    uint8_t          *premaster;   // dynamically allocated premaster buffered
    size_t            pms_len;     // length of premaster data 
    size_t            pms_size;    // Size of allocated data 
} ssl_kemctx_t; 
 ...
 */
    ...
    mbed_pktop_t peer_pubkey;     /*!< The public key from the peer.  */
...
int  mbedtls_ssl_handshake_kem_init (ssl_kemctx_t *kem_ctx ); 
...
unsigned char mbedtls_ssl_sig_from_pk_alg( mbed_hybpk_t type );
...
#if defined(MBEDTLS_X509_CRT_PARSE_C)
static inline mbed_pktop_t *mbedtls_ssl_own_key( mbedtls_ssl_context *ssl )
{
