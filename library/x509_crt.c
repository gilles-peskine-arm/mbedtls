                              ...
                              const mbedtls_x509_time *now)
{
    ...
    while (crl_list != NULL) {
        ...
        if (mbedtls_pk_verify_ext((mbedtls_pk_type_t) crl_list->sig_pk,
                                  crl_list->sig_opts, &ca->pk,
                                    ...
                                    mbedtls_x509_crt_restart_ctx *rs_ctx)
{
    ...
    return mbedtls_pk_verify_ext((mbedtls_pk_type_t) child->sig_pk,
                                 child->sig_opts, &parent->pk,
                                 child->sig_md, hash, hash_len,
                                             ...
                                             mbedtls_x509_crt_restart_ctx *rs_ctx)
{
    ...
    *flags = 0;
    ...
    switch (pk_type) {
        ...
        case MBEDTLS_PK_ECDSA:  /* shouldn't happen? */
            pk_alg = MBEDTLS_PK_SIGALG_ECDSA;
            break;
        case MBEDTLS_PK_ECKEY_DH:
            /* Mimic the historical behavior from Mbed TLS <=3.x.
             * This is probably not right: we shouldn't accept a
             * public key that's designated as ECDH-only here. */
            pk_alg = MBEDTLS_PK_SIGALG_ECDSA;
            break;
        default:
            /* This is a key of an unexpected type. Fall back to the bit
             * for MBEDTLS_PK_NONE in the profile->allowed_pks bitmask. */
            break;
    }

    if (x509_profile_check_pk_alg(profile, pk_alg) != 0) {
