    ...
    mbedtls_rsa_context **p_rsa )
{
    ...
    mbed_pktop_t ctx;
    size_t bits;
    mbed_pk_init( &ctx, NULL, NULL, NULL) ;

    /* Parse the data. */
    if( PSA_KEY_TYPE_IS_KEY_PAIR( type ) )
        status = mbedtls_to_psa_error(
            mbed_pk_parse_prvkey( ctx, data, data_length, NULL, 0,
                mbedtls_psa_get_random, MBEDTLS_PSA_RANDOM_STATE ) );
    else
        status = mbedtls_to_psa_error(
            mbed_pk_parse_pubkey( &ctx, data, data_length ) );
    if( status != PSA_SUCCESS )
        goto exit;
                                         ...
                                         size_t *data_length )
{
    ...
    if( PSA_KEY_TYPE_IS_KEY_PAIR( type ) )
        ret = mbed_pk_write_key_der( pk, data, data_size );
    else
        ret = pkey_write_pubkey( pk, &pos, data, );
