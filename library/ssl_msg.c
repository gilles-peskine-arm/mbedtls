...
int mbedtls_ssl_prepare_handshake_record(mbedtls_ssl_context *ssl)
{
...
#endif /* MBEDTLS_SSL_PROTO_DTLS */
    {
        ...
        int ret;
        const size_t hs_remain = ssl->in_hslen - ssl->handshake->in_hsfraglen;
        MBEDTLS_SSL_DEBUG_MSG(3,
                              ("handshake fragment: %" MBEDTLS_PRINTF_SIZET " .. %"
                               MBEDTLS_PRINTF_SIZET " of %"
                               MBEDTLS_PRINTF_SIZET " msglen %" MBEDTLS_PRINTF_SIZET,
                               ssl->handshake->in_hsfraglen,
                               ssl->handshake->in_hsfraglen +
                               (hs_remain <= ssl->in_msglen ? hs_remain : ssl->in_msglen),
                               ssl->in_hslen, ssl->in_msglen));
        if (ssl->in_msglen < hs_remain) {
            ssl->handshake->in_hsfraglen += ssl->in_msglen;
            ssl->in_hdr = ssl->in_msg + ssl->in_msglen;
            ssl->in_msglen = 0;
            mbedtls_ssl_update_in_pointers(ssl);
            return MBEDTLS_ERR_SSL_CONTINUE_PROCESSING;
        }
        if (ssl->handshake->in_hshdr != ssl->in_hdr) {
            /*
             * At ssl->handshake->in_hshdr we have a sequence of records that cover the next handshake
            ...
            unsigned char *p = ssl->handshake->in_hshdr, *q = NULL;
            do {
                mbedtls_record rec;
            ...
            ssl->in_hdr = ssl->handshake->in_hshdr;
            mbedtls_ssl_update_in_pointers(ssl);
        ...
        if (ssl->handshake->in_hsfraglen != 0) {
            ...
            MBEDTLS_SSL_DEBUG_MSG(3,
                                  ("waiting for more fragments (%" MBEDTLS_PRINTF_SIZET " of %"
                                   ...
                                   ssl->in_hslen - ssl->handshake->in_hsfraglen));
            return 0;
        }
