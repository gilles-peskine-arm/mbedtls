...
hmac_failed_etm_enabled:
    ...
    if (auth_done == 0) {
        ...
        if (transform->minor_ver > MBEDTLS_SSL_MINOR_VERSION_0) {
             ...
             * position of the MAC, through timing. This includes the
             * timing of memory accesses, which can leak to a local
             * attacker. Hence we use the function mbedtls_ct_hmac()
             * which is specially crafted to verify a HMAC at an
             * unknown position within a range.
             *
            ...
            switch (mode) {
                ...
                case MBEDTLS_MODE_STREAM:
                     ...
                     * We use a constant-trace HMAC here to avoid making this
                     * function even more complex. In terms of performance,
                     * with min_len==max_len, the overhead is negligible over
                     * mbedtls_md_hmac() + mbedtls_ct_memcmp(). */
                    min_len = max_len;
                    break;
