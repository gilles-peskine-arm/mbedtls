...
void mbedtls_x509write_crt_set_subject_key( mbedtls_x509write_cert *ctx, mbed_pktop_t *pktop_ref )
{
    ctx->subject_key = pktop_ref;
}

void mbedtls_x509write_crt_set_issuer_key( mbedtls_x509write_cert *ctx, mbed_pktop_t *pktop_ref )
{
    ctx->issuer_key = pktop_ref;
}
...
int mbedtls_x509write_crt_set_subject_key_identifier( mbedtls_x509write_cert *ctx )
{
    int ret = MBEDTLS_ERR_ERROR_CORRUPTION_DETECTED;
  #ifdef MBEDTLS_PQ_C
    unsigned char buf[ MBEDTLS_PK_HYBKEY_MAX_SIZE];   
  #else 
    unsigned char buf[(MBEDTLS_MPI_MAX_SIZE * 2 + 20)*2]; /* tag, length + 2xMPI */
  #endif 
    unsigned char *c = buf + sizeof(buf);
    size_t len = 0;

    memset( buf, 0, sizeof(buf) );
    MBEDTLS_ASN1_CHK_ADD( len, mbed_pk_write_key_der( ctx->subject_key, buf, sizeof(buf) ) );
...
int mbedtls_x509write_crt_set_authority_key_identifier( mbedtls_x509write_cert *ctx )
{
    ...
    memset( buf, 0, sizeof(buf) );
    MBEDTLS_ASN1_CHK_ADD( len, mbed_pk_write_pubkey_der(ctx->issuer_key, buf, sizeof( buf) ));
...
int mbedtls_x509write_crt_der( mbedtls_x509write_cert *ctx, unsigned char *buf, size_t size,  int (*f_rng)(void *, unsigned char *, size_t), void *p_rng)
{
     ...
     * (represented as an element of mbed_hybpk_t) from a PK instance. */
    if( mbed_pk_can_do( ctx->issuer_key, MBEDTLS_PK_RSA ) )
        pk_alg = MBEDTLS_PK_RSA;
    else if( mbed_pk_can_do( ctx->issuer_key, MBEDTLS_PK_ECDSA ) )
        pk_alg = MBEDTLS_PK_ECDSA;
 #ifdef MBEDTLS_PQ_C
    else if (mbed_pk_can_do( ctx->issuer_key, MBEDTLS_PK_PQDSA ))
        pk_alg = MBEDTLS_PK_PQDSA;
    else if (mbed_pk_can_do( ctx->issuer_key, MBEDTLS_PK_HYBDSA ) )
        pk_alg = MBEDTLS_PK_HYBDSA;
 #else 
    #error 
     ...
     */
    MBEDTLS_ASN1_CHK_ADD( pub_len, mbed_pk_write_pubkey_der( ctx->subject_key, buf, c - buf ) );
    c -= pub_len;
    len += pub_len;
    ...
    if( ( ret = mbed_pk_sign( ctx->issuer_key, ctx->md_alg, hash, 0, sig, sizeof( sig ), &sig_len) ) != 0 )
    {
        return( ret );
    }
...
int mbedtls_x509write_crt_pem( mbedtls_x509write_cert *crt, unsigned char *buf, size_t size,  int (*f_rng)(void *, unsigned char *, size_t), void *p_rng )
{
    ...
    if( ( ret = mbedtls_x509write_crt_der( crt, buf, size, NULL, NULL ) ) < 0 )
    {
        return( ret );
    }
