...
#define MAX_REQUEST_SIZE      2000000
...
int main( int argc, char *argv[] )
{
    ...
    if( strlen( opt.key_file ) )
        // TODO check RNG implementation 
        ret = mbed_pk_parse_prv_keyfile( pkey, opt.key_file, opt.key_pwd);
    else
#endif
        ret = mbed_pk_parse_prvkey( pkey, (const unsigned char *) mbedtls_test_cli_key, mbedtls_test_cli_key_len, NULL, 0);
    if( ret != 0 )
    {
    ...
    mbedtls_printf( "\n---------------------------------\n" );
    mbedtls_printf( " mbedtls_ssl_handshake OK\n");
    mbedtls_printf( "---------------------------------\n" );
