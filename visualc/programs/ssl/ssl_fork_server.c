...
int main( void )
{
    ...
    mbedtls_printf( " ok\n" );
    mbed_pk_init( &pkey, mbedtls_ctr_drbg_random, mbedtls_entropy_func, pers);
     ...
     * server and CA certificates, as well as mbed_pk_parse_prv_keyfile().
     */
    ...
    ret =  mbed_pk_parse_prvkey( pkey, (const unsigned char *) mbedtls_test_srv_key, mbedtls_test_srv_key_len, NULL, 0);
    if( ret != 0 )
    {
        mbedtls_printf( " failed!  mbed_pk_parse_prvkey returned %d\n\n", ret );
        goto exit;
    }
