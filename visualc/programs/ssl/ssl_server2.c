...
sni_entry *sni_parse( char *sni_string )
{
    ...
    *end = ',';
    ...
    while( p <= end )
    {
        ...
        if( mbed_pk_init( &new->key, mbedtls_ctr_drbg_random, NULL, pers) != 0)
            goto error;

        if( mbedtls_x509_crt_parse_file( new->cert, crt_file ) != 0 ||
            mbed_pk_parse_prv_keyfile( new->key, key_file, "") != 0 )
            goto error;
                            ...
                            size_t input_len )
{
     ...
     * we call mbed_pk_check_pair to match a private key with the
     * public key. */
    for( slot = 0; slot < config_data->slots_used; slot++ )
    {
        if( mbed_pk_check_pair( &cert->pk,config_data->slots[slot].pk) == 0 )
            break;
    }
                             ...
                             size_t output_size )
{
    ...
    switch( ctx->operation_type )
    {
        case ASYNC_OP_DECRYPT:
            ret = mbed_pk_decap( key_slot->pk,
                                      ctx->input, ctx->input_len,
                                      output, output_len, output_size,
                                      config_data->f_rng, config_data->p_rng );
            break;
        case ASYNC_OP_SIGN:
            ret = mbed_pk_sign( key_slot->pk,
                                   ctx->md_alg,
                                   ctx->input, ctx->input_len,
...
int main( int argc, char *argv[] )
{
    ...
    mbed_pktop_t *pkey = NULL;
    mbedtls_x509_crt srvcert2;
    mbed_pktop_t *pkey2 = NULL;
    ...
    mbed_pk_init(&pkey, mbedtls_ctr_drbg_random, NULL, pers );
    mbedtls_x509_crt_init( &srvcert2 );
    mbed_pk_init(&pkey2, mbedtls_ctr_drbg_random, NULL, pers );
#if defined(MBEDTLS_SSL_ASYNC_PRIVATE)
    ...
    {
        key_cert_init++;
        if( ( ret = mbed_pk_parse_prv_keyfile( pkey, opt.key_file, opt.key_pwd) ) != 0 )
        {
            mbedtls_printf( " failed\n  !  mbed_pk_parse_prv_keyfile returned -0x%x\n\n", (unsigned int) -ret );
            goto exit;
        }
    }
    if( key_cert_init == 1 )
    ...
    {
        key_cert_init2++;
        if( ( ret = mbed_pk_parse_prv_keyfile( pkey2, opt.key_file2, opt.key_pwd2) ) != 0 )
        {
            mbedtls_printf( " failed\n  !  mbed_pk_parse_prv_keyfile(2) returned -0x%x\n\n",
                            (unsigned int) -ret );
            goto exit;
        }
    }
        ...
        strcmp( opt.key_file2, "none" ) != 0 )
    {
        ...
        if( ( ret = mbed_pk_parse_prvkey( pkey, (const unsigned char *) mbedtls_test_srv_key_rsa, mbedtls_test_srv_key_rsa_len, NULL, 0) ) != 0 )
        {
            mbedtls_printf( " failed\n  !  mbed_pk_parse_prvkey returned -0x%x\n\n",
                            (unsigned int) -ret );
            goto exit;
        }
        key_cert_init = 2;
        ...
        if( ( ret = mbed_pk_parse_prvkey( pkey2, (const unsigned char *) mbedtls_test_srv_key_ec, mbedtls_test_srv_key_ec_len, NULL, 0) ) != 0 )
        {
...
exit:
    ...
    for( i = 0; (size_t) i < ssl_async_keys.slots_used; i++ )
    {
        ...
        {
            mbed_pk_free( ssl_async_keys.slots[i].pk );
