...
static void dump_pubkey( const char *title, mbedtls_ecdh_context *key )
{
...
int main( int argc, char *argv[] )
{
    ...
    dump_pubkey( "Client Public key: ", &ctx_cli);
    dump_point(  "Client Pubkey pnt: ", &ctx_cli.MBEDTLS_PRIVATE(grp), &ctx_cli.MBEDTLS_PRIVATE(Q)) ;
    dump_buf( "Client Pub key X : ", cli_to_srv, 32);
    ...
    mbedtls_printf( " ok\n" );
    dump_mpi( "Client shared secret: ", &ctx_cli.MBEDTLS_PRIVATE(z));
