...
int main( int argc, char *argv[] )
{
    ...
    else if( strlen( opt.ca_file ) )
    {
        ...
        mbedtls_printf( "  . Ca cert certificate information    ...\n" );
        ret = mbedtls_x509_crt_info( (char *) buf, sizeof( buf ) - 1, "      ", &cacert );
        if( ret == -1 )
        {
    ...
    if( opt.mode == MODE_FILE )
    {
         ...
         */
        mbedtls_printf( "\n  . Loading the certificate(s) '%s' ...",opt.filename );
        fflush( stdout );
    ...
    else if( opt.mode == MODE_SSL )
    {
         ...
         */
        mbedtls_printf( "  . SSL connection to tcp/%s/%s...", opt.server_name, opt.server_port );
        fflush( stdout );
...
#else
        mbedtls_printf( "  . Peer certificate information    ...\n" );
        ret = mbedtls_x509_crt_info( (char *) buf, sizeof( buf ) - 1, "      ", mbedtls_ssl_get_peer_cert( &ssl ) );
        if( ret == -1 )
        {
            mbedtls_printf( " failed\n  !  mbedtls_x509_crt_info returned %d\n\n", ret );
            goto ssl_exit;
