...
struct options
{
    ...
    mbed_md_alg_e md;        /* Hash used for signing                */
                       ...
                       void *p_rng )
{
    ...
    FILE *f;
  #ifdef MBEDTLS_PQ_C
    unsigned char output_buf[4*4096];
  #else 
    ...
    if( ( ret = mbedtls_x509write_crt_pem( crt, output_buf, sizeof(output_buf), NULL, NULL) ) < 0 )
        return( ret );
...
int main( int argc, char *argv[] )
{
    ...
    mbed_pktop_t *loaded_issuer_key = NULL, *loaded_subject_key = NULL;
    mbed_pktop_t *issuer_key, *subject_key;
    char buf[1024];
    ...
    mbed_pk_init( &loaded_issuer_key, mbedtls_ctr_drbg_random, mbedtls_entropy_func, pers);
    mbed_pk_init( &loaded_subject_key, mbedtls_ctr_drbg_random, mbedtls_entropy_func, pers);
    ...
    if( !opt.selfsign && strlen( opt.issuer_crt ) )
    {
        ...
        if( ( ret = mbedtls_x509_crt_parse_file( &issuer_crt, opt.issuer_crt ) ) != 0 )
        {
            ...
            mbedtls_printf( " failed\n  !  mbedtls_x509_crt_parse_file() '%s' "
                            "returned -0x%04x - %s\n\n", opt.issuer_crt, (unsigned int) -ret, buf );
            goto exit;
        }
    ...
    if( !opt.selfsign && strlen( opt.request_file ) )
    {
        ...
        subject_key = csr.MBEDTLS_PRIVATE(pktop_ref);
    ...
    if( !opt.selfsign && !strlen( opt.request_file ) )
    {
        ...
        ret = mbed_pk_parse_prv_keyfile( loaded_subject_key, opt.subject_key, opt.subject_pwd);
        if( ret != 0 )
        {
            mbedtls_strerror( ret, buf, 1024 );
            mbedtls_printf( " failed\n  !  mbed_pk_parse_prv_keyfile "
                            "returned -0x%04x - %s\n\n", (unsigned int) -ret, buf );
            goto exit;
        }
    ...
    ret = mbed_pk_parse_prv_keyfile( loaded_issuer_key, opt.issuer_key, opt.issuer_pwd);
    if( ret != 0 )
    {
        mbedtls_strerror( ret, buf, 1024 );
        mbedtls_printf( " failed\n  !  mbed_pk_parse_prv_keyfile "
    ...
    {
        if( mbed_pk_check_pair( issuer_crt.MBEDTLS_PRIVATE(pktop), issuer_key) != 0 )
        {
     ...
     */
    mbedtls_printf( "  . Writing the certificate to file '%s' ...", opt.output_file );
    fflush( stdout );
...
exit:
    ...
    mbed_pk_free( loaded_subject_key );
    mbed_pk_free( loaded_issuer_key );
