...
struct options
{
    ...
    mbed_md_alg_e md_alg;   /* Hash algorithm used for signature.   */
} opt;
                               ...
                               void *p_rng )
{
    ...
    if( ( ret = mbedtls_x509write_csr_pem( req, output_buf, 4096, NULL, NULL) ) < 0 )
        return( ret );
...
int main( int argc, char *argv[] )
{
    ...
    mbed_pk_init( &key, mbedtls_ctr_drbg_random, mbedtls_entropy_func, pers);
    ...
    ret = mbed_pk_parse_prv_keyfile( key, opt.filename, opt.password);

    if( ret != 0 )
    {
        mbedtls_printf( " failed\n  !  mbed_pk_parse_prv_keyfile returned %d", ret );
        goto exit;
    }
