...
static int tls_prf_generic( mbed_md_alg_e md_type,
                               ...
                               const mbedtls_ssl_context *ssl )
{
...
#if defined(MBEDTLS_USE_PSA_CRYPTO) &&          \
    defined(MBEDTLS_KEM_PSK_ENABLED)
    if( handshake->ciphersuite_info->key_exchange == MBEDTLS_KEM_PSK &&
        ...
        ssl_use_opaque_psk( ssl ) == 1 )
    {
        ...
        psa_key_derivation_operation_t derivation =
            PSA_KEY_DERIVATION_OPERATION_INIT;
        mbed_md_alg_e hash_alg = handshake->ciphersuite_info->mac;
...
#endif
    {
        ret = handshake->tls_prf( handshake->kem_ctx.premaster, handshake->kem_ctx.pms_len,
                                  lbl, salt, salt_len,
                                  master,
                                  master_secret_len );
        if( ret != 0 )
        ...
        MBEDTLS_SSL_DEBUG_BUF( 3, "premaster secret",
                               handshake->kem_ctx.premaster,
                               handshake->kem_ctx.pms_len );

        mbedtls_platform_zeroize( handshake->kem_ctx.premaster,
                                  sizeof(handshake->kem_ctx.premaster) );
    }
...
int mbedtls_ssl_psk_derive_premaster( mbedtls_ssl_context *ssl, mbedtls_kem_type_t key_ex )
{
    unsigned char *p = ssl->handshake->kem_ctx.premaster;
    unsigned char *end = p + sizeof( ssl->handshake->kem_ctx.premaster );
    ...
    if( key_ex == MBEDTLS_KEM_ECDHE_PSK )
    {
        int ret = MBEDTLS_ERR_ERROR_CORRUPTION_DETECTED;
        size_t zlen;
    ...
    *(p++) = (unsigned char)( psk_len      );
    ...
    ssl->handshake->kem_ctx.pms_len = p - ssl->handshake->kem_ctx.premaster;
 ...
 * Handshake functions
 */
#if !defined(MBEDTLS_KEM_WITH_CERT_ENABLED)
...
#else /* MBEDTLS_KEM_WITH_CERT_ENABLED */
                                         ...
                                         size_t crt_buf_len )
{
    ...
    mbed_md_alg_e const peer_cert_digest_type =
                                         ...
                                         void *rs_ctx )
{
    ...
    {
        mbed_pktop_t          *pk = chain->pktop;
        mbedtls_ecp_group_id   grp_id;

        /* If certificate uses an EC key, make sure the curve is OK */
        // TODO fix keyidx
        mbed_pk_get_ec_grpid(pk, 0, &grp_id); 
        if( mbed_pk_can_do_single( pk, MBEDTLS_PK_ECKEY)) 
        {
            if ( mbedtls_ssl_check_curve( ssl, grp_id ) != 0 )
            {
                printf("%s(): MBEDTLS_X509_BADCERT_BAD_KEY mark4\n", __func__);
                                     ...
                                     unsigned char *start, size_t len )
{
    ...
    ret = pkey_parse_subpubkey( &start, end,
...
#endif /* MBEDTLS_KEM_WITH_CERT_ENABLED */
...
int mbedtls_ssl_write_finished( mbedtls_ssl_context *ssl )
{
    ...
    MBEDTLS_SSL_DEBUG_MSG( 2, ( "=> write finished 2 " ) );
    ...
    MBEDTLS_SSL_DEBUG_MSG( 2, ( "<= write finished RH " ) );
...
static int ssl_handshake_init( mbedtls_ssl_context *ssl )
{
    ...
    mbedtls_ssl_handshake_kem_init( &ssl->handshake->kem_ctx);
                             ...
                             size_t len )
{
    ...
    session->peer_cert_digest_type = (mbed_md_alg_e) *p++;
...
#if defined(MBEDTLS_X509_CRT_PARSE_C) &&        \
    !defined(MBEDTLS_SSL_KEEP_PEER_CERTIFICATE)
    mbed_pk_free( &handshake->peer_pubkey );
...
void mbedtls_ssl_free( mbedtls_ssl_context *ssl )
{
    ...
    if( ssl->handshake )
    {
        mbedtls_ssl_handshake_kem_free( &ssl->handshake->kem_ctx );
 ...
 */
unsigned char mbedtls_ssl_sig_from_pk( mbed_pktop_t *pk )
{
#if defined(MBEDTLS_RSA_C)
    if( mbed_pk_can_do( pk, MBEDTLS_PK_RSA ) )
        return( MBEDTLS_SSL_SIG_RSA );
#endif
#if defined(MBEDTLS_ECDSA_C)
    if( mbed_pk_can_do( pk, MBEDTLS_PK_ECDSA ) )
        return( MBEDTLS_SSL_SIG_ECDSA );
#endif
#if defined(MBEDTLS_PQ_C)
    if (mbed_pk_can_do( pk, MBEDTLS_PK_PQDSA ))
        return( MBEDTLS_SSL_SIG_PQ );
    if (mbed_pk_can_do( pk, MBEDTLS_PK_HYBDSA ))
        return (MBEDTLS_SSL_SIG_HYBRID);
#endif
    return( MBEDTLS_SSL_SIG_ANON );
}

unsigned char mbedtls_ssl_sig_from_pk_alg( mbed_hybpk_t hybpk_alg )
{
    switch( hybpk_alg ) {
        case MBEDTLS_PK_RSA:
            return( MBEDTLS_SSL_SIG_RSA );
      ...
      #ifdef MBEDTLS_PQ_C
        case MBEDTLS_PK_HYBDSA:
            return (MBEDTLS_SSL_SIG_HYBRID);
        case MBEDTLS_PK_PQDSA:
            return( MBEDTLS_SSL_SIG_PQ );
      #endif 
...
mbed_hybpk_t mbedtls_ssl_pk_alg_from_sig( unsigned char sig )
{
    switch( sig )
    {
#if defined(MBEDTLS_RSA_C)
...
#endif
  #if defined(MBEDTLS_PQ_C)
        case MBEDTLS_SSL_SIG_PQ:
            return( MBEDTLS_PK_PQDSA );
        case MBEDTLS_SSL_SIG_HYBRID:
            return( MBEDTLS_PK_HYBDSA );
  #endif
        default:
...
mbed_md_alg_e mbedtls_ssl_sig_hash_set_find( mbedtls_ssl_sig_hash_set_t *set, mbed_hybpk_t sig_alg )
{
    switch( (uint32_t)sig_alg )
    {
        case MBEDTLS_PK_RSA:
            return( set->rsa );
        case MBEDTLS_PK_ECDSA:
                                   ...
                                   mbed_md_alg_e md_alg )
{
    switch( (uint32_t)sig_alg )
    {
     ...
     #ifdef MBEDTLS_PQ_C
        case MBEDTLS_PK_PQDSA:
            if( set->pqdsa == MBEDTLS_MD_NONE )
                set->pqdsa = md_alg;
            break;
        case MBEDTLS_PK_HYBDSA:
            if( set->hybdsa == MBEDTLS_MD_NONE )
                set->hybdsa = md_alg;
            break;
     #endif 
 ...
 */
mbed_md_alg_e mbedtls_ssl_md_alg_from_hash( unsigned char hash )
{
    switch( hash )
    {
                          ...
                          uint32_t *flags )
{
    ...
    if( cert_endpoint == MBEDTLS_SSL_IS_SERVER )
    {
        ...
        {
            case MBEDTLS_KEM_RSA:
            case MBEDTLS_KEM_RSA_PSK:
                usage = MBEDTLS_X509_KU_KEY_ENCIPHERMENT;
                break;

            case MBEDTLS_KEM_DHE_RSA:
            case MBEDTLS_KEM_ECDHE_RSA:
            case MBEDTLS_KEM_ECDHE_ECDSA:
                usage = MBEDTLS_X509_KU_DIGITAL_SIGNATURE;
                break;

            case MBEDTLS_KEM_ECDH_RSA:
            case MBEDTLS_KEM_ECDH_ECDSA:
                usage = MBEDTLS_X509_KU_KEY_AGREEMENT;
                break;

          #if defined( MBEDTLS_PQ_C)
            case MBEDTLS_KEM_PQKEM_PQDSA:
            case MBEDTLS_KEM_HYBKEM_HYBDSA:
                usage = MBEDTLS_X509_KU_DIGITAL_SIGNATURE | MBEDTLS_X509_KU_KEY_AGREEMENT;
                break;
          #endif // MBEDTLS_PQ_C

            /* Don't use default: we want warnings when adding new values */
            case MBEDTLS_KEM_NONE:
            case MBEDTLS_KEM_PSK:
            case MBEDTLS_KEM_DHE_PSK:
            case MBEDTLS_KEM_ECDHE_PSK:
            case MBEDTLS_KEM_ECJPAKE:
                usage = 0;
        }
    }
    else
