...
int ssl_pkwrap_write_server_key_exchange( mbedtls_ssl_context  *ssl ); 
int ssl_pkwrap_parse_client_key_exchange( mbedtls_ssl_context  *ssl ); 
#endif 
                                               ...
                                               size_t len )
{
    ...
    mbed_md_alg_e md_cur;
    mbed_hybpk_t sig_cur;
...
static int ssl_check_key_curve( mbed_pktop_t *pktop,
                                const mbedtls_ecp_curve_info **curves )
{
    const mbedtls_ecp_curve_info **crv = curves;
    mbedtls_ecp_group_id         grp_id;

    // TODO: Fix the keyidx = 0 
    mbed_pk_get_ec_grpid( pktop, 0, &grp_id);
...
static int ssl_pick_cert( mbedtls_ssl_context *ssl, const mbedtls_ssl_ciphersuite_t * ciphersuite_info )
{
    ...
    mbed_hybpk_t pk_alg = mbedtls_ssl_get_ciphersuite_sig_pk_alg( ciphersuite_info );
    uint32_t flags;
    ...
    for( cur = list; cur != NULL; cur = cur->next )
    {
        ...
        if( ! mbed_pk_can_do( cur->cert->pktop, pk_alg ) )
        {
...
#if defined(MBEDTLS_ECDSA_C)
        if( pk_alg == MBEDTLS_PK_ECDSA && ssl_check_key_curve( cur->cert->pktop, ssl->handshake->curves ) != 0 )
        {
                                  ...
                                  const mbedtls_ssl_ciphersuite_t **ciphersuite_info )
{
    ...
    if( ssl->minor_ver == MBEDTLS_SSL_MINOR_VERSION_3 )
    {
        ...
        {
            if( mbedtls_ssl_sig_hash_set_find( &ssl->handshake->hash_algs, sig_type ) == MBEDTLS_MD_NONE )
            {
...
read_record_header:
    ...
    {
        mbed_md_alg_e md_default = MBEDTLS_MD_SHA1;
...
have_ciphersuite:
    ...
    if( ssl->minor_ver == MBEDTLS_SSL_MINOR_VERSION_3 )
    {
        mbed_hybpk_t sig_alg = mbedtls_ssl_get_ciphersuite_sig_alg( ciphersuite_info );
        if( sig_alg != MBEDTLS_PK_NONE )
        {
            mbed_md_alg_e md_alg = mbedtls_ssl_sig_hash_set_find( &ssl->handshake->hash_algs, sig_alg );
            MBEDTLS_SSL_DEBUG_MSG( 3, ( "client hello v3, signature_algorithm ext: %d", mbedtls_ssl_hash_from_md_alg( md_alg ) ) );
        }
        else
        {
...
static int ssl_get_ecdh_params_from_cert( mbedtls_ssl_context *ssl )
{
    ...
    // TODO: 
    printf("%s(): EC KEY only of mbed_pk_can_do() !!!!!\n"); 
    if( ! mbed_pk_can_do( mbedtls_ssl_own_key( ssl ), MBEDTLS_PK_ECKEY ) )
    {
        MBEDTLS_SSL_DEBUG_MSG( 1, ( "server key not ECDH capable" ) );
        return( MBEDTLS_ERR_SSL_PK_TYPE_MISMATCH );
    }

    if( ( ret = mbedtls_ecdh_get_params( &ssl->handshake->ecdh_ctx, mbed_get_eckey_ctx( *mbedtls_ssl_own_key( ssl ) ),
                                 MBEDTLS_ECDH_OURS ) ) != 0 )
    {
...
#endif /* defined(MBEDTLS_KEM_WITH_SERVER_SIGNATURE_ENABLED) &&
          defined(MBEDTLS_SSL_ASYNC_PRIVATE) */
...
static int ssl_prepare_server_key_exchange( mbedtls_ssl_context *ssl, size_t *signature_len )
{
...
#if defined(MBEDTLS_KEM_SOME_PFS_ENABLED)
#if defined(MBEDTLS_KEM_WITH_SERVER_SIGNATURE_ENABLED)
    unsigned char *dig_signed = NULL;
#endif /* MBEDTLS_KEM_WITH_SERVER_SIGNATURE_ENABLED */
#endif /* MBEDTLS_KEM_SOME_PFS_ENABLED */
     ...
     * - DHE key exchanges
     */
#if defined(MBEDTLS_KEM_SOME_DHE_ENABLED)
...
#endif /* MBEDTLS_KEM_SOME_DHE_ENABLED */

    /*
     * - ECDHE key exchanges
     */
#if defined(MBEDTLS_KEM_SOME_ECDHE_ENABLED)
...
#endif /* MBEDTLS_KEM_SOME_ECDHE_ENABLED */
...
#if defined(MBEDTLS_SSL_PROTO_TLS1_2)
        mbed_hybpk_t sig_alg = mbedtls_ssl_get_ciphersuite_sig_pk_alg( ciphersuite_info );
        if( ssl->minor_ver == MBEDTLS_SSL_MINOR_VERSION_3 )
        {
            /*    For TLS 1.2, obey signature-hash-algorithm extension
...
#error 
         ...
         * ssl_write_server_key_exchange also takes care of incrementing
         * ssl->out_msglen. */
        if( ( ret = mbed_pk_sign( mbedtls_ssl_own_key( ssl ), md_alg, hash, hashlen, ssl->out_msg + ssl->out_msglen + 2,
...
#endif /* MBEDTLS_KEM_SOME_ECDH_ENABLED */
...
{
    int ret = MBEDTLS_ERR_ERROR_CORRUPTION_DETECTED;
    mbed_pktop_t *private_key = mbedtls_ssl_own_key( ssl );
    mbed_pktop_t *public_key = &mbedtls_ssl_own_cert( ssl )->pk;
    size_t len = mbed_pk_get_bitlen_single( public_key)+7/8;
    ...
    printf("%s(): RSA key ONLY use of mbed_pk_can_do() !!!!!\n"); fflush(stdout); 
    if( ! mbed_pk_can_do( private_key, MBEDTLS_PK_RSA ) )
    {
        MBEDTLS_SSL_DEBUG_MSG( 1, ( "got no RSA private key" ) );
        return( MBEDTLS_ERR_SSL_PRIVATE_KEY_REQUIRED );
    }

    ret = mbed_pk_decap( private_key, p, len,
                              peer_pms, peer_pmslen, peer_pmssize,
                              ssl->conf->f_rng, ssl->conf->p_rng );
    return( ret );
}
...
#pragma warning( pop )
    ...
    for( i = 0; i < ssl->handshake->kem_ctx.pms_len; i++ )
...
#if defined(MBEDTLS_SSL_ASYNC_PRIVATE) && \
    ( defined(MBEDTLS_KEM_RSA_ENABLED) || \
      defined(MBEDTLS_KEM_RSA_PSK_ENABLED) )
    if( ( ciphersuite_info->key_exchange == MBEDTLS_KEM_RSA_PSK ||
          ciphersuite_info->key_exchange == MBEDTLS_KEM_RSA ) &&
        ( ssl->handshake->async_in_progress != 0 ) )
    {
...
static int ssl_parse_certificate_verify( mbedtls_ssl_context *ssl )
{
    ...
    if( ( ret = mbed_pk_verify( peer_pk, md_alg, hash_start, hashlen, ssl->in_msg + i, sig_len ) ) != 0 )
    {
...
int mbedtls_ssl_handshake_server_step( mbedtls_ssl_context *ssl )
{
    ...
    switch( ssl->state )
    {
        ...
        case MBEDTLS_SSL_SERVER_KEY_EXCHANGE:
          #ifdef KEM_REWORK 
            ret = ssl_pkwrap_write_server_key_exchange( ssl );
          #else 
        ...
        case MBEDTLS_SSL_CLIENT_KEY_EXCHANGE:
          #ifdef KEM_REWORK 
            ret = ssl_pkwrap_parse_client_key_exchange( ssl );
          #else 
