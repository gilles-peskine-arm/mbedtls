...
#endif
    2048,
    2,      /* min_nist_level, this should apply to all algorithms */ 
};
 ...
 */
static int x509_profile_check_pk_alg( const mbedtls_x509_crt_profile *profile, mbed_hybpk_t pk_alg )
{
 ...
 */
static int x509_profile_check_key( const mbedtls_x509_crt_profile *profile, mbed_pktop_t *pktop)
{
    mbed_pktype_e  pk_alg;
    size_t         bitlen;
    int            num_keys; // Number of keys in the hybrid key context 
    int            i; 

    
    /*---------------------Code ----------------------------------------------*/
    if ( mbed_pk_get_numkeys( pktop, &num_keys) != SUCCESS) 
        return -1; 

    for (i=0; i< num_keys; i++)
    {
        if ( mbed_pk_get_type( pktop, i, &pk_alg) != SUCCESS ) 
            return -1; 

      #if defined(MBEDTLS_RSA_C)
        if( pk_alg == MBEDTLS_PK_RSA || pk_alg == MBEDTLS_PK_RSASSA_PSS )
        {
             if( mbed_pk_get_bitlen_single( pktop, i, &bitlen ) != SUCCESS)
                 return -1;
            if( bitlen >= profile->rsa_min_bitlen )
                continue;  // This key is supported, check the next key 
        ...
        if( pk_alg == MBEDTLS_PK_ECDSA || pk_alg == MBEDTLS_PK_ECKEY || pk_alg == MBEDTLS_PK_ECKEY_DH )
        {
            ...
            mbed_pk_get_ec_grpid(pktop, i, &gid);
        ...
        {
            if (mbed_pk_get_nist_level( pktop, i ) < profile->min_nist_level)
                return -1;
        }
      #endif
    }
                                    ...
                                    void *p_ctx )
{
  ...
  #if 0 
    uint8_t *saved_p;
    // Loop through all public keys. (TODO: Cludge fix me) 
    // Stop when parsing of the key fails.  
    while (1 )
    {
        saved_p = p; 
        util_dump( p, len, "raw cert data"); 
        if ((ret = mbed_pk_parse_subpubkey( crt->pktop, &p, end )) != 0 )
            break;  
    }
    p = saved_p; 
    if( p == crt->pk_raw.p) // no key parsed  
        goto fail; 
 #else 
    if ((ret = mbed_pk_parse_subpubkey( crt->pktop, &p, end )) != 0 )
        goto fail;  
                            ...
                            size_t buflen )
{
    ...
    if( buf_format == MBEDTLS_X509_FORMAT_PEM )
    {
        ...
        while( buflen > 1 )
        {
            ...
            /* If get there, we know the string is null-terminated */
...
int mbedtls_x509_crt_parse_file( mbedtls_x509_crt *chain, const char *path )
{
    ...
    if( ( ret = mbed_pk_load_file( path, &buf, &n ) ) != 0 )
        return( ret );
...
int mbedtls_x509_crt_info( char *buf, size_t size, const char *prefix, const mbedtls_x509_crt *crt )
{
    ...
    /* Key size */
    if( ( ret = mbedtls_x509_key_size_helper( key_size_str, BEFORE_COLON, mbed_pk_get_hybrid_name( crt->pktop ) ) ) != 0 )
    {
        return( ret );
    }
    // TODO: informational only 
    bit_len = mbed_pk_get_bitlen( crt->pktop);
    ret = mbedtls_snprintf( p, n, "\n%s%-" BC "s: %d bits", prefix, key_size_str,(int) bit_len );
    MBEDTLS_X509_SAFE_SNPRINTF;
    ...
    ret = mbedtls_snprintf( p, n, "\n%ssubject key info : \n", prefix );
    MBEDTLS_X509_SAFE_SNPRINTF;
    ret = mbed_pk_export_key_info( crt->pktop, PK_TARGET_PUB, PK_EXPORT_BASIC, p, n); 
    MBEDTLS_X509_SAFE_SNPRINTF;
  #endif 
                               ...
                               const mbedtls_x509_crt_profile *profile )
{
    ...
    while( crl_list != NULL )
    {
        ...
        if( x509_profile_check_key( profile, ca->pktop ) != 0 ) {
            flags |= MBEDTLS_X509_BADCERT_BAD_KEY;
        }

        // TODO 
       //if( mbed_pk_verify_ext_old( crl_list->sig_pk, crl_list->sig_opts, &ca->pk,
        //                   crl_list->sig_md, hash, mbedtls_md_get_size( md_info ),
        //                  crl_list->sig.p, crl_list->sig.len ) != 0 )
        if( mbed_pk_verify_ext_old( ca->pktop, crl_list->sig_md, hash, mbedtls_md_get_size( md_info ),
                                crl_list->sig.p, crl_list->sig.len, crl_list->sig_opts ) != 0 )
        {
            flags |= MBEDTLS_X509_BADCRL_NOT_TRUSTED;
            break;
        }
                                     ...
                                     mbedtls_x509_crt_restart_ctx *rs_ctx )
{
    ...
    //if( ! mbedtls_pk_can_do( parent->pktop, child->sig_pk ) )
    //    return( -1 );
    if( strcmp( mbed_pk_get_hybrid_name(parent->pktop), mbed_pk_get_hybrid_name(child->pktop) ) != 0) 
        return( -1 );

#if defined(MBEDTLS_ECDSA_C) && defined(MBEDTLS_ECP_RESTARTABLE)
    if( rs_ctx != NULL && child->sig_pk == MBEDTLS_PK_ECDSA )
    {
        return( mbed_pk_verify_restartable( &parent->pktop,
    ...
    // TODO check this 
    return( mbed_pk_verify_ext_old( parent->pktop, child->sig_md, hash, hash_len, child->sig.p, child->sig.len, child->sig_opts ) );
}
...
find_parent:
 ...
 #error 
        ...
        if( x509_profile_check_key( profile, parent->pktop ) != 0 ) {
            *flags |= MBEDTLS_X509_BADCERT_BAD_KEY;
        }
                                  ...
                                  uint32_t *flags )
{
    ...
    else
    {
        for( name = &crt->subject; name != NULL; name = name->next )
        {
            ...
            strncpy(list_name, (char*)name->val.p, name->val.len);
                     ...
                     mbedtls_x509_crt_restart_ctx *rs_ctx )
{
    ...
    *flags = 0;
    ...
    // TODO 
    mbed_pk_get_type( crt->pktop, 0, &pktype);

    if( x509_profile_check_pk_alg( profile, pktype ) != 0 )
        ee_flags |= MBEDTLS_X509_BADCERT_BAD_PK;
    
    if( x509_profile_check_key( profile, crt->pktop ) != 0 ) {
...
void mbedtls_x509_crt_init( mbedtls_x509_crt *crt )
{
    ...
    mbed_pk_init( &crt->pktop, NULL, NULL, NULL); 
}
