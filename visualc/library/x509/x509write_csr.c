...
void mbedtls_x509write_csr_set_key( mbedtls_x509write_csr *ctx, mbed_pktop_t *pktop )
{
    ctx->pktop = pktop;
}
                                       ...
                                       unsigned char *sig, size_t sig_size)
{
    ...
    MBEDTLS_ASN1_CHK_ADD( pub_len, mbed_pk_write_pubkey_der( ctx->pktop, buf, c - buf ) );
    c -= pub_len;
    len += pub_len;
    ...
    if( ret != 0 )
        return( ret );
#endif
    if ((ret = mbed_pk_sign(ctx->pktop, ctx->md_alg, hash, sizeof(hash), sig, sig_size, &sig_len)) != 0)
    {
        return( ret );
    }

    // Get the signature OID 
    if ( (ret = mbed_pk_get_signature_oid(ctx->pktop, ctx->md_alg, &sig_oid, &sig_oid_len )) != 0) 
        return ret; 
    /*
...
int mbedtls_x509write_csr_der( mbedtls_x509write_csr *ctx, unsigned char *buf, size_t size, int (*f_rng)(void *, unsigned char *, size_t), void *p_rng)
{
    ...
    ret = x509write_csr_der_internal( ctx, buf, size, sig, MBEDTLS_PK_SIGNATURE_MAX_SIZE);
...
int mbedtls_x509write_csr_pem( mbedtls_x509write_csr *ctx, unsigned char *buf, size_t size, int (*f_rng)(void *, unsigned char *, size_t), void *p_rng)
{
    ...
    if( ( ret = mbedtls_x509write_csr_der( ctx, buf, size, NULL, NULL) ) < 0 )
    {
        return( ret );
    }
